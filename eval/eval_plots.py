...
def load_csv(path):
    ...
    with open(path, encoding='utf-8') as f:
        for r in csv.DictReader(f):
            ...
            for f in ('cam', 'bs_p', 'bs_r', 'bs_f1'):
                r[f] = float(r[f]) if r.get(f) not in (None, '', 'None') else None
...
# ─────────────────────────────────────────────────────────────────────────────
# FIGURE 1a/1b/1c — Métriques globales, UNE figure PAR CATÉGORIE
# ─────────────────────────────────────────────────────────────────────────────
# Bug de design fixé 2026-07-08 : l'ancienne fig_global_metrics mettait EXM,
# chrF, P@1, KGH, CTA, TAM, NEG sur UN SEUL graphique avec un axe 0-100
# partagé — ces métriques ne mesurent pas la même chose (similarité de
# surface vs justesse sémantique vs conformité structurelle) et n'ont
# jamais été destinées à être comparées entre elles sur une même barre.
# Remplacé par 3 figures séparées, une par catégorie, chacune ne comparant
# que des métriques de même nature.
def _bar_metric_fig(labels, values, color, title, ylabel, outname, outdir):
    ...
    ax.set_title(title)
    _save(fig, outdir, outname)


def fig1a_surface_metrics(rows, outdir):
    """EXM, chrF, chrF++, BLEU (char) — toutes des mesures de similarité de
    surface texte-à-texte (hypothèse vs référence), comparables entre elles."""
    from evaluate import corpus_metrics
    n = len(rows)
    exm  = sum(r['exm'] for r in rows) / n * 100
    chrf = sum(r['chrf'] for r in rows) / n
    corp = corpus_metrics([r['output'] for r in rows], [r['reference'] for r in rows])
    labels = ['EXM', 'chrF', 'chrF++', 'BLEU\n(char)']
    values = [exm, chrf, corp.get('chrfpp_corpus') or 0.0, corp.get('bleu_corpus') or 0.0]
    _bar_metric_fig(labels, values, C_BLUE,
                     f'Métriques de surface — Kuma-MT (n={n})',
                     'Score (0–100)', 'fig1a_surface_metrics', outdir)


def fig1b_semantic_metrics(rows, outdir):
    """P@1, KGH, CamemBERT, BERTScore F1 — toutes des mesures de justesse
    sémantique (le sens est-il préservé ?), comparables entre elles."""
    n = len(rows)
    p1  = _avg_pct([r['p1']  for r in rows if r['p1']  is not None])
    kgh = _avg_pct([r['kgh'] for r in rows if r['kgh'] is not None])
    cam = _avg_pct([r.get('cam')   for r in rows if r.get('cam')   is not None])
    f1  = _avg_pct([r.get('bs_f1') for r in rows if r.get('bs_f1') is not None])
    labels = ['P@1', 'KGH', 'CamemBERT', 'BERTScore\nF1']
    values = [p1, kgh, cam, f1]
    _bar_metric_fig(labels, values, C_ORANGE,
                     f'Métriques sémantiques — Kuma-MT (n={n})',
                     'Score (%)', 'fig1b_semantic_metrics', outdir)


def fig1c_structure_metrics(rows, outdir):
    """CTA, TAM, NEG, ordre S-TAM-V — toutes des mesures de conformité
    grammaticale/structurelle, comparables entre elles."""
    n = len(rows)
    ...
    wo  = _avg_pct([r['word_order_ok'] for r in rows if r.get('word_order_ok') is not None])
    labels = ['CTA', 'TAM', 'NEG', 'Ordre\nS-TAM-V']
    values = [cta, tam, neg, wo]
    _bar_metric_fig(labels, values, C_GREEN,
                     f'Métriques structurelles — Kuma-MT (n={n})',
                     '% conforme', 'fig1c_structure_metrics', outdir)
...
def fig_ud_health(rows, outdir):
    ...
    if not valid:
        print("    Pas de données UD — fig9 ignorée")
        return
...
def fig_radar(rows, outdir, parser_json=None):
    ...
    # NOTE (bug fixé 2026-07-08) : r['chrf'] est DÉJÀ sur l'échelle 0-100
    # (score chrF), et r['p1']/r['kgh'] sont DÉJÀ passés en % avant appel à
    # _p() — _avg_pct()/_p() multiplient pourtant par 100 en plus,
    # produisant des valeurs ~9700-9900% au lieu de 97-99%. Avec l'axe
    # polaire limité à (0, 100), ces coordonnées hors bornes faisaient
    # planter le rendu matplotlib (segfault) plutôt que d'être simplement
    # clippées visuellement.
    metrics = {
        'EXM':    _avg_pct([r['exm']  for r in rows]),
        'chrF':   sum(r['chrf'] for r in rows) / len(rows) if rows else 0.0,
        'P@1':    _avg_pct([r['p1']  for r in rows if r['p1']  is not None]),
        'KGH':    _avg_pct([r['kgh'] for r in rows if r['kgh'] is not None]),
...
def fig_word_order(rows, outdir, mapping):
    ...
    if not valid:
        print("    Pas de données word_order — fig11 ignorée")
        return
...
def main():
    ...
    if parser_json:
        ...
        print("ℹ  Aucun parser_eval_*.json — figures parsing ignorées "
    ...
    if csv_path:
        ...
        try:
            ...
            mapping = {}
        # Figures traduction
        fig1a_surface_metrics(rows, args.outdir)
        fig1b_semantic_metrics(rows, args.outdir)
        fig1c_structure_metrics(rows, args.outdir)
        ...
        print("ℹ  Aucun eval_*.csv — figures traduction ignorées.")
