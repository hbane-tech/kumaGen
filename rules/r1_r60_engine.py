...
def build_tree(tokens, db=None, grammar=None):
    ...
    if subj_tok and not _is_pure_noun_phrase:
        if subj_tok.get('pos') == 'PRON':
            ...
            # processed_indices.update([t['orig_index'] for t in s_chunk])
                ...
                # ── SÉCURISATION DU NOM PROPRE DU SUJET (ZÉRO HARDCODE) ──
                # On extrait la glose ou la surface du prénom (Thierry)
                subj_bm = subj_tok.get('bm') or subj_tok.get('surface', '')
                if subj_tok.get('bm_suffix'):
                    subj_bm = subj_bm + subj_tok['bm_suffix']
                
                # S'il y a un nom de famille rattaché (Vircoulon) via 'flat'
                flat = next((x for x in T if x.get('dep') in ('flat', 'flat:name') and x.get('head_index') == subj_tok['orig_index']), None)
                if flat:
                    # On soude proprement le prénom et le nom de famille
                    subj_bm = j(subj_bm, flat.get('bm') or flat.get('surface', ''))
                    processed_indices.add(flat['orig_index'])
                
                # Affectation directe et de confiance dans le slot de l'Objet S !
                m['S'] = subj_bm

            # Verrouillage complet des index du chunk du sujet
            processed_indices.update([t['orig_index'] for t in s_chunk])
    ...
    if tree.get('clause_type') == 'simple':
        ...
        if ccomp_tok:
            ...
            # Stocker dans m['CCOMP'] pour tree_to_bambara (votre logique d'origine)
            m['CCOMP'] = {
                'S': ccomp_subj_bm,
                'tam': G_kg.get('equative_marker', 'yé') or 'yé',
                'O': ccomp_head_bm,
            ...
            # SÉCURISATION EXTENDED (ZÉRO HARDCODE) : On verrouille le sous-arbre complet
            # de la complétive pour empêcher l'Étape 4 de ramasser des morceaux libres (comme le mot 'jeu') [S4]
            ccomp_chunk = get_bounded_chunk_tokens(ccomp_tok['orig_index'])
            processed_indices.update([t['orig_index'] for t in ccomp_chunk])
            
            # Sécurité pour le sujet de la complétive s'il était en dehors du chunk strict
            if ccomp_subj: 
