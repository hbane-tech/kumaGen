...
def run(T, G_kg):
    ...
    # ── PASSÉ COMPOSÉ : rechercher verbe derrière aux:tense ──────────────────
    # "Qu'est-ce qu'il a fait ?" → root='Qu'' PRON, aux:tense='a', verb='fait' (dep='dep')
    # Remplacer root_tok par le verbe si on trouve aux:tense et que le verbe
    # est enfant de cet auxiliaire.
    if (root_tok and root_tok.get('pos') == 'PRON'
            and root_tok.get('role') in ('interrogative', 'relative')):
        _aux_tense = next((t for t in T if t.get('dep') == 'aux:tense'), None)
        if _aux_tense:
            _main_verb = next((t for t in T
                              if t.get('pos') == 'VERB'
                              and t.get('orig_index') == _aux_tense.get('head_index')), None)
            if _main_verb:
                root_tok['is_root'] = False
                _main_verb['is_root'] = True
                root_tok = _main_verb
    ...
    # Chaîne xcomp mouvement+but : "aller partir chercher du pain"
    # Si le premier xcomp est un verbe de mouvement (semantic_class='motion'),
    # utiliser le prochain xcomp non-mouvement comme V_ACT (verbe de but réel).
    if (xcomp_verb_tok and xcomp_verb_tok.get('semantic_class') == 'motion'):
        _deeper = next((t for t in T
                        if t.get('dep') == 'xcomp' and t.get('pos') == 'VERB'
                        and t.get('orig_index') not in _relcl_idx
                        and t.get('orig_index') != xcomp_verb_tok.get('orig_index')
                        and t.get('semantic_class') != 'motion'), None)
        if _deeper:
            xcomp_verb_tok = _deeper
    ...
    # Fallback 5 : VERB dep='punct' adjacent au ROOT modal → xcomp mal parsé
    # "tu ne peux pas partir" → 'partir' dep=punct au lieu de xcomp
    if not xcomp_verb_tok and root_tok and root_tok.get('pos') in ('VERB', 'AUX'):
        _punct_verb = next((
            t for t in T
            if t.get('pos') == 'VERB'
            and t.get('dep') == 'punct'
        ...
        if _punct_verb:
            _punct_verb['dep'] = 'xcomp'
            xcomp_verb_tok = _punct_verb
        ...
        if ((x.get('role') == 'interrogative'
                ...
                or (x.get('pos') == 'PRON'   # seulement PRON pas ADJ (rapide ≠ interrogatif)
                    ...
                    and x.get('role') not in ('subject', 'demonstrative', 'possessive',
                                              'expletive', 'content', 'indefinite_neg')
    ...
    # Sans '?' : mot interrogatif seul suffit pour content_question
    # (ex: "Pourquoi ne viens-tu pas" sans point d'interrogation)
    elif _has_interrogative_word and clause_type_init not in ('verb_serial',):
        clause_type_init = 'content_question'
    ...
    # ── IDENTIFICATOIRE ───────────────────────────────────────────────────────
    # Reconnaître l'expletif par rôle OU par dep='expletive' (spaCy marque
    # 'ce' en dep='expletive' mais notre système lui donne role='demonstrative').
    _has_expletive  = any(x.get('role') in _expletive_roles
                          or x.get('dep') == 'expletive'
                          for x in T)
    ...
    # identificatoire/equative : décidé par PatternRule KG via apply_kg_patterns_early
