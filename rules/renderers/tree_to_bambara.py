...
def tree_to_bambara(tree, G=None, grammar=None):
    ...
    if (ct in ('simple', 'complex', 'conditional', 'temporal', 'relative_post')
        ...
        if _alt_tok:
            ...
            if _v_is_motion and _v_display:
                ...
                _is_action_interrog = (
                    ...
                    and _root_tok_ref2
                    and _root_tok_ref2.get('intransitive_type') in ('ACTION', 'nominalized', 'support')
                    and _root_tok_ref2.get('semantic_class', '') not in INTRANS_SC)
        ...
        # V+li kɛ réservé aux transitifs sans COD (semantic_class ∉ INTRANS_SC),
        # et jamais au négatif ("je ne mange pas" → n tɛ dún).
        # travailler=having ∈ INTRANS_SC → exclu ici, traité comme intransitif (V la) plus bas.
        _is_action_verb = (V and not O
                           and ct == 'simple'
                           and not tree.get('neg')
                           and not _root_is_resultative
                           and _root_tok_sc not in INTRANS_SC
        ...
        _is_intrans_action = (V and not O
                              and ct == 'simple'
                              and not _root_is_resultative
                              and _root_tok_sc in INTRANS_SC
                              and _root_intrans == 'ACTION')
        if _is_action_verb:
            _v_nom = V + 'li'
            if _has_com_obl:
            ...
            if _is_intrans_action:
                # Progressif intransitif ACTION → n bɛ kà báara kɛ
                result = j(*_tmp_strs, S, TAM, O, V, 'kɛ', ADV, *_other_strs)
            else:
                result = j(*_tmp_strs, S, TAM, O, V, ADV, *_other_strs)
        elif _is_intrans_action and TAM in ('bɛ', 'tɛ'):
            # Présent intransitif ACTION → n bɛ báara la
            result = j(_contrast_str, S, TAM, V, 'la', V_ACT, V_SUF,
        ...
        _refl_toks = tree.get('_tokens', [])
        _rec_root  = next((t for t in _refl_toks if t.get('is_root')), None)
        _rec_plural = _rec_root.get('is_plural', False) if _rec_root else False
        _refl_marker = 'ɲɔgɔn' if _rec_plural else j(S, 'yɛrɛ')
        # Passé réciproque : S yé ɲɔgɔn V  (objet présent → transitif → yé, pas -ra)
        if tn == 'past':
            _rec_tam = 'ma' if neg else 'yé'
            result = j(S, _rec_tam, _refl_marker, V, *obl_strings)
        else:
            result = j(S, TAM, _refl_marker, V, *obl_strings)
        ...
        if _qty_interrog and _qty_interrog.get('bm') not in (O or ''):
            ...
            result = j(O, _exist_op, S, _poss_marker, *obl_strings, _polar_end)
